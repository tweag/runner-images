...
# Change waagent entries to use /mnt for swap file on Azure VMs
# On AWS you can do something similar with CloudInit https://cloudinit.readthedocs.io/en/latest/reference/yaml_examples/mounts.html
if [[ -f /etc/waagent.conf ]]; then
    sed -i 's/ResourceDisk.MountPoint=.*/ResourceDisk.MountPoint=\/mnt/g' /etc/waagent.conf
